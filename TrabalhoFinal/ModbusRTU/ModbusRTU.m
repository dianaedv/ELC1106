...
tempo_simulacao = 50; % tempo de simulacao
...
function [NovosEventos] = executa_evento(evento, tempo_atual)
    global msg, global rede, global nos, global tabela;
    ...
    switch tipo_evento
        case 'N_cfg' % configura nos, inicia variaveis de estado, etc.
            ...
            pct =  struct('src', id, 'dst', 0, 'tam', 2, 'codigo', 03, 'dado', 107,'registrador',1, 'crc',7687);
      ...
      case 'R_ini' %inicio de recepcao
       ...
       case 'Resposta do servidor'
              if (id ==2) % servidor id=1
            pct =  struct('src', id, 'dst', 0, 'tam', 2, 'codigo', 03, 'dado', 107, 'crc',7687);

            e = evento_monta(tempo_atual+rand(1) , 'T_fim', id, pct);
            NovosEventos =[NovosEventos;e];
          end
